...
class  HouseCount:
    ...
    def __init__(self,AreaCode, Blockcode):
        ...
        countnvnet = max_pool_2d(convnet, 5)
        ...
        for v in range(0,79):
            if v <10:
                img_data = cv2.imread("./area_zip" +str(x) + "/" + "img0"+str(v)+".jpg",cv2.IMREAD_GRAYSCALE)
            else:
                img_data = cv2.imread("./area_zip" +str(x) + "/" + "img"+str(v)+".jpg",cv2.IMREAD_GRAYSCALE)
